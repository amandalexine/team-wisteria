# --------------------------------SAVE FUNCTIONS-----------------------------------------
# Description:
#   This module handles saving various plots, machine learning results,
#   and statistical analyses to an Excel file using OpenPyXL.
#
# Functions:
#   - save_graphs_to_excel(): Saves general statistics plots
#   - save_ml_graphs_to_excel(): Saves machine learning signal plots
#   - save_ml_results_to_excel(): Saves ML classification results and features
#   - save_stats_results_to_excel(): Saves statistical summaries
#
# Dependencies:
#   openpyxl, io, os, matplotlib (for figures)
# ---------------------------------------------------------------------------------------
...
def save_graphs_to_excel(excel_file, graphs_dict):
    """
    Saves statistical result plots to an Excel worksheet called "Stats Graphs".

    Args:
        excel_file (str): Path to the Excel file.
        graphs_dict (dict): Dictionary where keys are section names (e.g., 'ECG')
    ...
    # check if workbook exists (if so, open it, or create a new one)
    ...
    # Create or refresh "Stats Graphs" tab/sheet within the workbook
    ...
    # Iterate through each section and its list of figures
    for section, fig_list in graphs_dict.items():
        if not fig_list:
            continue
            
        # Write section title in bold
        cell = ws.cell(row=current_row, column=1, value=section)
        cell.font = Font(bold=True, size=12)  
        current_row += 1

        # Save each figure to a BytesIO buffer and insert into Excel
...
def save_ml_graphs_to_excel(excel_file, graphs_dict):
    """
    Saves machine learning-related signal plots to an Excel tab/sheet called "Signal Graphs".

    Args:
        excel_file (str): Path to Excel file.
        graphs_dict (dict): Nested dictionary structured as:
                            { category_name: { data_type: matplotlib.figure } }
    ...
    # Create or clear out the target worksheet
    ...
    # go through each (category, data type) pair
...
def save_ml_results_to_excel(excel_file, ml_prediction, ml_features):
    """
    Saves machine learning classification results and related feature data
    to an Excel sheet named "ML Results".

    Args:
        excel_file (str): Path to Excel file.
        ml_prediction (dict): Dictionary containing model outputs, e.g.
                              {'ecg': {'classification': int, 'confidence': float, 'fig': matplotlib.figure}}
        ml_features (dict): Feature data structured as:
                            { signal_type: {'baseline_data': {}, 'test_data': {}, 'percent_difference': {}} }
    ...
    # make a sheet / tab if it doesnt exists or clear it out
    ...
    # Write feature comparison data if available
    if ml_prediction['ecg'] is not None:
        ...
        for signal_type, categories in ml_features.items():
            ...
            for feature in feature_types: 
                ...
                # get feature or N/A if it doesn;t exist
        ...
        # Save SHAP waterfall plot if provided
...
def save_stats_results_to_excel(excel_file, stats_data):
    """
    Saves statistical analysis data to an Excel sheet titled "Stats Results".
    """
    ...
    for signal_type, categories in stats_data.items():
        ...
        baseline_data = categories.get('baseline') or {}
        test_data = categories.get('test') or {}
        diff_data = categories.get('diff') or {}
        flags_data = categories.get('flags') or {}
        ...
        if baseline_data:
            ...
            stat_types = list(baseline_data.keys())

            for stat in stat_types:
                curr_row += 1

                baseline_value = baseline_data.get(stat, 'N/A')
                test_value = test_data.get(stat, 'N/A')
                diff = diff_data.get(stat, 'N/A')
                flag = flags_data.get(stat, 'N/A')

                baseline_value = f"{baseline_value:.5f}" if isinstance(baseline_value, (int, float)) else 'N/A'
                test_value = f"{test_value:.5f}" if isinstance(test_value, (int, float)) else 'N/A'
                diff = f"{diff:.5f}%" if isinstance(diff, (int, float)) else 'N/A'
